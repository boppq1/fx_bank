...
{{
    ...
    "tendency": "목적외화형/편의환전형/환율수익형/고금리외화형"
}}
"""
...
def run_review_analysis():
    ...
    for i, (review_no, product_no, review_text, rating) in enumerate(reviews):
        ...
        print(f"\n[{i+1}/{len(reviews)}] 리뷰 번호 : {review_no}")
        print("-" * 60)
        print("입력 리뷰")
        print(review_text)
        print("-" * 60)
        ...
        try:
            ...
            product_sentiments[product_no].append(
            ...
            product_tendency_data[product_no][tendency] = \
                product_tendency_data[product_no].get(tendency, 0) + 1

        except Exception as e:
            print(f"❌ Error processing review {review_no}")
            print(e)
